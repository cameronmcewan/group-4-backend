...
contract Portfolio is ERC20 {
    ...
    event RedeemAssets(address indexed _from, uint256 _tokensBurned);
    ...
    function initialisePortfolio() public payable onlyOwner zeroTotalSupply {
        ...
        for (uint256 i = 0; i < tokenAddresses.length; i++) {
            ...
            uint256 numTokensAcquired = swapIn(
                WETH,
                tokenAddresses[i],
    ...
    function deposit(uint256 _totalWethAmount) private returns (uint256) {
        ...
        for (uint256 i = 0; i < tokenAddresses.length; i++) {
            ...
            priorValueLocked +=
                (assetQuantities[tokenAddresses[i]] * wethToSpend) /
                numTokensAcquired;
    ...
    function redeemAssets(uint256 tokensToSell) public nonZeroTotalSupply {
        ...
        for (uint256 i = 0; i < tokenAddresses.length; i++) {
            ...
            uint256 numTokensToWithdraw = (assetQuantity * tokensToSell) /
                prevSupply;
    ...
    function swapIn(
        ...
        if (tokenOut == WETH) {
            ...
            _numTokensAcquired = callUniswap(
                tokenIn,
                tokenOut,
                tokenInAmount,
                recipient
